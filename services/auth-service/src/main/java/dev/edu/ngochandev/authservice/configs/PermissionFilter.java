...
public class PermissionFilter extends OncePerRequestFilter {
    ...
    private boolean hasPermission(Authentication authentication, String requestURI, String requestMethod) {
        ...
        return permissions.stream()
                .anyMatch(per -> antPathMatcher.match(per.getApiPath(), requestURI)
                        && per.getMethod().name().equalsIgnoreCase(requestMethod));
    }
    ...
    private boolean isPublicEndpoint(String requestURI) {
        ...
        return Arrays.stream(publicEndpoints).anyMatch(endpoint -> antPathMatcher.match(endpoint, requestURI));
    }
}
