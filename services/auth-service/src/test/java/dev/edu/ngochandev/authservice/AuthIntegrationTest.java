...
public class AuthIntegrationTest  extends AbsIntegrationTest {
	...
	@Autowired
	public AuthIntegrationTest(ObjectMapper objectMapper,
							MockMvc mockMvc,
	...
	void registerNewUser_validData_success(TestInfo testInfo) throws Exception {
		...
		log.debug("[Assert] Response content: {}", response.andReturn().getResponse().getContentAsString());
	...
	void whenRegister_withDuplicateEmail_thenReturns409Conflict(TestInfo testInfo) throws Exception {
		...
		userRepository.save(UserEntity.builder().username("existing").email("duplicate.email@example.com").password("pwd").fullName("name").build());
	...
	void whenRefreshToken_withValidToken_thenReturns200AndNewTokens(TestInfo testInfo) throws Exception {
		...
		UserEntity savedUser = userRepository.save(UserEntity.builder().username("refresher").password(passwordEncoder.encode("pass123")).email("ref@test.com").fullName("n").build());
	...
	void whenLogout_withValidToken_thenInvalidatesToken(TestInfo testInfo) throws Exception {
		log.info("==========> STARTING TEST: {} <==========", testInfo.getDisplayName());
		// Arrange
		UserEntity savedUser = userRepository.save(UserEntity.builder().username("logoutuser").password(passwordEncoder.encode("pass123")).email("logout@test.com").fullName("n").build());
		assignDefaultRoleToUser(savedUser);
		...
		mockMvc.perform(post("/api/v1/users/list") // checking a protected endpoint
						.header("Authorization", "Bearer " + accessTokenToLogout)
						.contentType(MediaType.APPLICATION_JSON)
						.content("{}"))
...
//    helper method to get authentication tokens
	private void assignDefaultRoleToUser(UserEntity user){
		...
		userRole.setRole(roleRepository.findByName(DataInitializer.DEFAULT_ROLE).orElseThrow(() -> new RuntimeException("Default role not found")));
